...
$(document).ready(function() {
    ...
    function updateSubmitButton() {
        const rowCount = $('#requestTableBody tr').length;
        $('#submitRequestBtn').prop('disabled', rowCount === 0);
        $('#emptyMessage').toggle(rowCount === 0);
    }

    // Aksi ketika tombol "Tambah ke Daftar" diklik
    $('#addItemButton').on('click', function() {
        const itemId = $('#item_id_select').val();
        const itemName = $('#item_id_select option:selected').data('name');
        ...
        if (itemId && qty && parseInt(qty) > 0) {
            ...
            const newRow = `
                <tr id="row-${itemCounter}">
                    ...
                    <td>
                        <button type="button" class="btn btn-danger btn-sm remove-item" data-row-id="${itemCounter}">Hapus</button>
                    </td>
                </tr>
            `;

            $('#requestTableBody').append(newRow);
            ...
            alert('Silakan pilih barang dan masukkan kuantitas yang valid.');
        }
    });
    ...
    // Inisialisasi status tombol submit saat modal dibuka
    updateSubmitButton();
});
</script>
@endpush